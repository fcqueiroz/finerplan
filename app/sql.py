...
class SqliteOps(object):
    ...
    def sum_query(self, query_str, query_values):
    ...
    def transactions_table(self, kind=None, monthly=True, num=50):
        ...
        if monthly:
            data_range = ('WHERE accrual_date >= "' + self.SOCM.strftime(date_model) +
                          '" and accrual_date < "' + self.SOM.strftime(date_model) + '" ')
        else:
            data_range = ''
    ...
    def generate_categories(table='expenses'):
        ...
        if table in ['expenses', 'earnings']:
            ...
            try:
                ...
                super_cat = [(row[0], row[0]) for row in result]
